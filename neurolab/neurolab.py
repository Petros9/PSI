...
def cross_entropy(predicted_labels, labels):
    y = tf.math.log(predicted_labels)  # TODO: zaimplementuj mnie, pieknie prosze!
    y = tf.one_hot(labels, depth=y.shape[1]) * y
...
class TestChapterOne(unittest.TestCase):
    ...
    def test_exercise_three(self):
        ...
        w1 = tf.Variable(tf.random.truncated_normal(shape=[784, 100], stddev=0.1))  # TODO: zaimplementuj mnie, pieknie prosze!
        b1 = tf.Variable(tf.random.truncated_normal(shape=[100], stddev=0.1))  # TODO: zaimplementuj mnie, pieknie prosze!
        w2 = tf.Variable(tf.random.truncated_normal(shape=[100, 10], stddev=0.1))  # TODO: zaimplementuj mnie, pieknie prosze!
        b2 = tf.Variable(tf.random.truncated_normal(shape=[10], stddev=0.1))  # TODO: zaimplementuj mnie, pieknie prosze!
...
class TestChapterTwo(unittest.TestCase):
    ...
    def test_exercise_four(self):
        ...
        model = tf.keras.models.Sequential([
            ...
            tf.keras.layers.Dense(100, tf.nn.relu),  # TODO: zaimplementuj mnie, pieknie prosze!
            # TODO: zastap None (analogicznie), pamietaj o wlasciwych rozmiarach i aktywacji!
            # koncowa warstwa neuronow
            tf.keras.layers.Dense(10, tf.nn.softmax),  # TODO: zaimplementuj mnie, pieknie prosze!
        ])
        ...
        model.compile(
            ...
            loss=tf.keras.losses.categorical_crossentropy,  # TODO: zaimplementuj mnie, pieknie prosze!
            ...
            optimizer=tf.keras.optimizers.SGD(lr=0.1),  # TODO: zaimplementuj mnie, pieknie prosze!
...
class TestChapterThree(unittest.TestCase):
    def test_exercise_five(self):
        ...
        # [ODPOWIEDŹ] Sieć z poprzedniego zadania miała 79,510 parametrów, sieć z tego zadania ma 143,667,240 parametrów, więc jest około 1807 raza większa
        ...
        image_path = 'armata.jpg'
        image = tf.keras.preprocessing.image.load_img(
            image_path,
            target_size=(224, 224, 3),  # TODO: zaimplementuj mnie, pieknie prosze!
        )
        ...
        #[ODPOWIEDŹ] Program rozpoznał małpę prawidłowo
        # Wykorzystałem zdjęcie stołu z wazonem stojące przed oknem, program pokazał coś takiego: ('n04590129', 'window_shade', 0.3965952), ('n04081281', 'restaurant', 0.23942934), ('n03201208', 'dining_table', 0.094405025)
        # Zaklasyfikował więc obraz bardzo dobrze
        # Pokazałem też zdjecie armaty, która została uznana za armatę oraz traktor, być może sieć widząc podobne elementy (w tym przypadku koła) przypisuje zdjęcie do dwóch różnych kategorii
        ...
        # [ODPOWIEDŹ] Ilością wymiarów - na początku jest 3D potem 1D